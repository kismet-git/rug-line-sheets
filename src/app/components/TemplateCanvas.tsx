...
export default function TemplateCanvas({
  ...
  const slotWidth =
    maxColumns > 0 ? `calc((100% - ${(maxColumns - 1) * 1.5}rem) / ${maxColumns})` : "100%";
  let slotIndex = 0;

  return (
    <section className="print-area aspect-[17/11] mx-auto w-full max-w-[1600px] rounded-3xl bg-white p-10 shadow-2xl print:m-0 print:max-w-none print:rounded-none print:p-8 print:shadow-none">
      <div className="flex h-full flex-col">
        ...
        <div className="mt-8 flex flex-1 flex-col justify-center gap-6">
          {rows.map((count, rowIndex) => {
            ...
            return (
              <div key={`row-${rowIndex}`} className="flex w-full justify-center gap-6">
                {items.map((_, columnIndex) => {
                  const currentIndex = slotIndex;
                  const slot = slots[currentIndex] ?? EMPTY_SLOT;
                  slotIndex += 1;
                  return (
                    <div key={`slot-${rowIndex}-${columnIndex}`} className="flex flex-col" style={{ width: slotWidth }}>
                      <ImageSlot
                        index={currentIndex}
                        slot={slot}
